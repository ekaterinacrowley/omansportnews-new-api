...
watcher.on('all', (event, path) => {
  ...
  try { execSync('node scripts/inject-env-html.cjs', { stdio: 'inherit' }); } catch (e) { console.error('inject-env-html failed:', e && e.message ? e.message : e); }
  execSync('npx posthtml tmp/index.injected.html -o public/index.html -c posthtml.config.cjs', { stdio: 'inherit' });
  } catch (e) {
